...
function aprroverView(req,res)
{
    ...
    // var emp_name =req.user.rows[0].user_name;
            ...
            pool.query("SELECT comm_code_desc cocd ,emp_name emp, * from leaves l,common_code_tbl cocd , emp_master_tbl emp where  emp.del_flg ='N' and  l.del_flg='N' and l.approver_id ='1257' and l.rcre_user_id = emp.emp_id and  cocd.del_flg ='N'and cocd.comm_code_id = l.leave_type and cocd.code_id ='LTYP'",[emp_id],function(err,leavesList){
             if (err) {
                console.error('Error with table query', err);
            } else {
            const    leaveData = leavesList.rows;
                console.log('leaveData value',leaveData);
              //  console.log(leaveData[0].emp_id);
                      
               pool.query('select emp_name from emp_master_tbl where emp_id=$1',[emp_id],function(err,result){
                if(err) throw error
                
               const emp_name=result.rows[0].emp_name
                res.json({message:"redirect to apprv/Reject",leaveData:leaveData,emp_name:emp_name});
               })
            }
